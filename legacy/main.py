...
def getLinksCourse(link):
    ...
    linkToStages = '{}{}'.format(link, '/steps')
    html_fetch = requests.get(linkToStages)
    
    soup_fetch = BeautifulSoup(html_fetch.text, "html.parser")
    # Find all urls of the videos (ignore reviews and questions)
    videos = []
    for a in soup_fetch.select('a[href^="/library/"]'):
        if (a.select('.video-22-icon')):  # if video icon is there
            video_link = '{}{}'.format('https://teamtreehouse.com', a['href'])
...
def download_project_file(soup, downloaded_zips):
    ...
    if zip_files:
        ...
        if not project_zip in downloaded_zips:
            ...
            if int(filesize)/(1024*1024) > 1000000:
...
for link in open('links.txt'):
    ...
    try:
        ...
        for video in videos:
            ...
            download_project_file(soup, zips)
